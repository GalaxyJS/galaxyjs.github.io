...
// Mixins
  ...
  function resolveMixins(mixins, output) {
    ...
    while (index--) {
      ...
      if (xtag.mixins[mixins[index]].mixins)
        resolveMixins(xtag.mixins[mixins[index]].mixins, output);
    }
    return output;
  }
            ...
            case 'prototype':
              for (var z in item) {
                if (!original[z])
                  ...
                  mergeMixin(tag, original[z], item[z], name);
              }
              break;
            default:
              mergeMixin(tag, original, item, name);
          }
        }
      }
    });
...
// Events
  ...
  function delegateAction(pseudo, event) {
    ...
    if (!match && root.tagName && matchSelector.call(root, pseudo.value))
      match = root;
  ...
  var xtag = {
    ...
    register: function (name, options) {
      ...
      if (typeof name == 'string')
        ...
        throw 'First argument must be a Custom Element string name';
      ...
      for (z in lifecycle)
        lifecycle[z.split(':')[0]] = xtag.applyPseudos(z, lifecycle[z], tag.pseudos, lifecycle[z]);
      for (z in tag.methods)
        proto[z.split(':')[0]] = {value: xtag.applyPseudos(z, tag.methods[z], tag.pseudos, tag.methods[z]), enumerable: true};
      for (z in tag.accessors)
        parseAccessor(tag, z);

      if (tag.shadow)
        tag.shadow = tag.shadow.nodeName ? tag.shadow : xtag.createFragment(tag.shadow);
      if (tag.content)
        tag.content = tag.content.nodeName ? tag.content.innerHTML : parseMultiline(tag.content);
      ...
      proto.createdCallback = {
        ...
        value: function () {
          var element = this;
          if (tag.shadow && hasShadow)
            this.createShadowRoot().appendChild(tag.shadow.cloneNode(true));
          if (tag.content)
            this.appendChild(document.createElement('div')).outerHTML = tag.content;
          ...
          if (finalized)
            finalized.apply(this, arguments);
          return output;
        }
      };
      ...
      if (lifecycle.attributeChanged)
        proto.attributeChangedCallback = {value: lifecycle.attributeChanged, enumerable: true};
      ...
      proto.setAttribute = {
        ...
        value: function (name, value) {
          ...
          if (attr) {
            if (attr.setter)
              attr.setter.call(this, attr.boolean ? true : value, old);
      ...
      proto.removeAttribute = {
        ...
        value: function (name) {
          ...
          if (attr) {
            if (attr.setter)
              attr.setter.call(this, attr.boolean ? false : undefined, old);
      ...
      if (basePrototype)
        Object.getOwnPropertyNames(basePrototype).forEach(function (z) {
          var prop = proto[z];
          var desc = instance ? Object.getOwnPropertyDescriptor(basePrototype, z) : basePrototype[z];
          if (prop) {
            for (var y in desc) {
              if (typeof desc[y] == 'function' && prop[y])
                prop[y] = xtag.wrap(desc[y], prop[y]);
              else
                prop[y] = desc[y];
            }
          }
        ...
        condition: function (event, custom) {
          if (event.type == 'pointerdown') {
            ...
            custom.startY = event.clientY;
                  ...
                  Math.abs(custom.startY - event.clientY) < 10)
            return true;
        }
      },
      tapstart: {
    ...
    skipFrame: function (fn) {
      var id = xtag.requestFrame(function () {
        id = xtag.requestFrame(fn);
      });
      return id;
    },
    ...
    createFragment: function (content) {
      ...
      if (content) {
        if (content.nodeName)
          toArray(arguments).forEach(function (e) {
            template.content.appendChild(e);
          });
        else
          template.innerHTML = parseMultiline(content);
      }
